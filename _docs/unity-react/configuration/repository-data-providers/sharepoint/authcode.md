...
App registration certificate and application grants also used as system read only user to cache SP metadata.  
 ...
 | ApplicationId | Required. Azure App registration applicationId(aka clientId) | `<ApplicationId>f1d7c8bc-6284-4db8-968f-e88a9bca70e1</ApplicationId>` |
 ...
 | OAuthMode | Optional. `[redirect|popup]`. Defaults to `redirect`. `popup` - Azure interaction in a separate popup window, `redirect` - Azure interaction in the same Unity window. | `<OAuthMode>redirect</OAuthMode>` |
