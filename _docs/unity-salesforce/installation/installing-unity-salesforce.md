...
- Enter the information below to create the new tab and click `Save`:
  ...
  | Tab Name            | Should be matched with Tab Id that was configured in the Unity application. This ID can be used only for one Tab. Template IDS setup are under 40 characters and contain underscores. For example, we cannot use sharepoint-documents-template in unity. It must be formatted as sharepoint_documents-templates | search_templates |
